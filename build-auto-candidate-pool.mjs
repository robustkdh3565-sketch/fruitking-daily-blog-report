...
if (snapshotFiles.length < 2) throw new Error(`SNAPSHOT_COVERAGE_LOW ${snapshotFiles.length}/3`);
const minimumCandidates = snapshotFiles.length === 3 ? 5 : 3;
if (candidates.length < minimumCandidates) throw new Error(`AUTO_CANDIDATE_POOL_LOW ${candidates.length}/${minimumCandidates}`);
const payload = { schemaVersion: 1, reportDate: report.reportDate, snapshotDate, generatedAt: new Date().toISOString(), snapshotFiles: snapshotFiles.map((file) => path.basename(file)), snapshotCoverage: snapshotFiles.length === 3 ? 'complete' : 'partial-no-imputation', candidates };
